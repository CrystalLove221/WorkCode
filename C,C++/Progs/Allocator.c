...
//	struct Allocation_t* structAlloc = (struct Allocation_t*) alloc;
	for(int i = 0; i < this->listSize; i++)
	{
		...
		{
			this->memUsed -= getSize(this->allocList[i]);
			free(this->allocList[i]);
			this->allocList[i] = NULL;
...
void* riskyAlloc(struct Allocator_t* this, size_t allocSize)
{
	...
	void* mem = allocate(this, allocSize);
	...
	void* tempMem = this->memory;
	this->memory = realloc(this->memory, this->capacity + allocSize);
	...
	this->capacity = this->capacity + allocSize;
	return allocate(this, allocSize);
}
